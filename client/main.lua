...
SendCurrentVehicleDataToNui = function(updateData)
    ...
    local vehData = {
        ...
        neon = IsVehicleNeonLightEnabled(currVeh, 0) or 
               IsVehicleNeonLightEnabled(currVeh, 1) or 
               IsVehicleNeonLightEnabled(currVeh, 2) or 
               IsVehicleNeonLightEnabled(currVeh, 3)
    }
...
RegisterNuiCallback("vehmenu:toggleoption", function(data, cb)
    ...
    if option == "neon" then
        local vehicleClass = GetVehicleClass(currVeh)
    
        -- Vehicle class 8 is for motorcycles (bikes)
        if vehicleClass == 8 then
            Debug("Neon lights not available for bikes.")
            return
        end
    
        local neonLeft, neonRight, neonFront, neonBack = GetVehicleNeonLightsColour(currVeh)
        local neonState = IsVehicleNeonLightEnabled(currVeh, 0) -- Check if front neon is enabled
        local newState = not neonState
    
        for i = 0, 3 do
            SetVehicleNeonLightEnabled(currVeh, i, newState)
        end
    
        Debug("Neon lights set to: ", newState)
    end
...
AddEventHandler('vehmenu:toggleoptionn1', function()
    SendCurrentVehicleDataToNui()
end)